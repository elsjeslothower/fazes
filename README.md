...
[`scripts/generate-icons.js`](scripts/generate-icons.js) — a color wheel on a
dark backdrop, with each phase's wedge sized to its share of the cycle
(pulled directly from `phaseSegments()` in
[`src/cycle/phaseEngine.js`](src/cycle/phaseEngine.js), the same function the
dashboard's progress bar uses, so the icon can't drift out of sync with the
real phase proportions). No external image tools required. Regenerate with
`npm run icons`. Swap in a real branded icon later if you want — a
